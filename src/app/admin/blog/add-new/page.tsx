...
const postSchema = z.object({
  ...
  slug: z.string().min(5, { message: 'Slug must be at least 5 characters long.' }).regex(/^[a-z0-9-]+$/, 'Slug can only contain lowercase letters, numbers, and hyphens.'),
...
export default function AddNewPostPage() {
  ...
  const handleFormSubmit = async (values: PostFormValues, status: 'Published' | 'Draft') => {
    const isLoading = status === 'Published';
    if(isLoading) setIsPublishing(true); else setIsSavingDraft(true);
    ...
    try {
        ...
        if (result.success) {
         ...
         toast({
            title: 'Error',
            description: error.message || `Could not save the ${status.toLowerCase()}.`,
            variant: 'destructive',
        });
    } finally {
       if(isLoading) setIsPublishing(false); else setIsSavingDraft(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit((values) => handleFormSubmit(values, 'Published'))} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex flex-col sm:flex-row gap-2 pt-4">
                  <Button type="button" variant="outline" className="w-full" onClick={() => form.handleSubmit((values) => handleFormSubmit(values, 'Draft'))()} disabled={isSavingDraft}>
