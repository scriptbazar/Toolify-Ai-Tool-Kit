  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <Button asChild>
            <Link href={`/admin/users/${user.uid}`}>Edit Profile</Link>
          </Button>
      </div>

       <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <Badge variant="outline" className="p-2 justify-center"><CheckCircle className="h-4 w-4 mr-2 text-green-500"/>Active</Badge>
            <Badge variant="outline" className="p-2 justify-center"><UserCog className="h-4 w-4 mr-2 text-primary"/>{profile.role}</Badge>
            <Badge variant="outline" className="p-2 justify-center"><ShieldCheck className="h-4 w-4 mr-2 text-green-500"/>KYC Verified</Badge>
            <Badge variant="outline" className="p-2 justify-center"><CalendarDays className="h-4 w-4 mr-2 text-muted-foreground"/>Joined {new Date(profile.createdAt.seconds * 1000).toLocaleDateString()}</Badge>
            <Badge variant="outline" className="p-2 justify-center"><UserIcon className="h-4 w-4 mr-2 text-primary"/>User ID: {user.uid.slice(0,8)}...</Badge>
        </div>
        ...
        <Tabs defaultValue="overview">
            <TabsList>
                ...
                <TabsTrigger value="withdrawals" disabled>Withdrawals</TabsTrigger>
                <TabsTrigger value="wallet" disabled>Wallet Management</TabsTrigger>
            </TabsList>
            <TabsContent value="overview" className="mt-6">
               <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  ...
                  <StatCard title="Last Transaction" value="2d ago" note="on 8/3/2025" icon={CalendarDays}/>
               </div>
            </TabsContent>
        </Tabs>
    </div>
