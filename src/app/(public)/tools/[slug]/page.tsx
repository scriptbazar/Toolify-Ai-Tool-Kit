...
export default function ToolPage() {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                const [settingsData, allPostsData, allToolsData] = await Promise.all([
                    getSettings(),
                    getPosts(),
                    getTools(),
                ]);

                const currentTool = allToolsData.find(t => t.slug === slug);
                ...
                const reviewsData = await getReviews({ toolId: slug });

                setTool(currentTool);
                setSettings(settingsData);
                setToolReviews(reviewsData);
                
                const popular = allToolsData.filter(t => t.status === 'Active' && t.slug !== slug).slice(0, 10);
                const recent = allPostsData.filter(p => p.status === 'Published').slice(0, 10);
