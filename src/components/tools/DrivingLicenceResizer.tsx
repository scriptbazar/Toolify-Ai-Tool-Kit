...
const PHOTO_WIDTH = 826;
const PHOTO_HEIGHT = 1062;
...
const ImageResizerBox = ({
  ...
  const handleResize = () => {
    ...
    img.onload = () => {
      ...
      if (dataUrl.length / 1.33 < minSizeKb * 1024) {
           toast({ title: "Warning", description: `Result is smaller than ${minSizeKb}KB. This might be due to a low resolution source image.`, variant: "default" });
      }
  ...
  const handleDownload = () => {
      ...
      link.download = `driving-licence-${title.toLowerCase()}-${Date.now()}.jpeg`;
      link.click();
  }
