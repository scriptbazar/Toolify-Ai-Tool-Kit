...
export function MeeshoShippingLabelCropper() {
  ...
  const handleCrop = async () => {
    ...
    try {
      ...
      link.download = `cropped-meesho-${pdfFile.name}`;
      link.click();
      URL.revokeObjectURL(link.href);
      
      toast({ title: 'Success!', description: 'Your Meesho shipping label has been cropped.' });
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="text-sm text-muted-foreground">{pdfFile ? `Selected: ${pdfFile.name}` : 'Click or drag Meesho label PDF to upload'}</p>
