 ...
 const handleMerge = async () => {
    ...
    try {
        ...
        for (const fileItem of files) {
            if (!fileItem.totalPages) {
                console.warn(`Skipping file ${fileItem.file.name} as it could not be read.`);
                continue;
            }
            ...
            const pagesToCopyIndices = parsePages(fileItem.pages, pdfDoc.getPageCount()).map(p => p - 1);
