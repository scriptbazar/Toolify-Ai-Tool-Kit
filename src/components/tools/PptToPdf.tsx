...
export function PptToPdf() {
  ...
  const handleFile = (selectedFile: File) => {
    if (selectedFile && (selectedFile.type === 'application/vnd.ms-powerpoint' || selectedFile.type === 'application/vnd.openxmlformats-officedocument.presentationml.presentation')) {
      setFile(selectedFile);
    } else {
      toast({ title: 'Invalid File', description: 'Please upload a valid PowerPoint file (.ppt, .pptx).', variant: 'destructive' });
    }
  };
        ...
        >
            ...
            >
                ...
                <div className="flex flex-col items-center justify-center h-full">
                    ...
                    <h3 className="text-lg font-semibold">{file ? file.name : "Click or drag PowerPoint file to upload"}</h3>
                    <p className="text-sm text-muted-foreground">Your presentation will be converted to a PDF.</p>
                </div>
            </CardContent>
       </Card>
