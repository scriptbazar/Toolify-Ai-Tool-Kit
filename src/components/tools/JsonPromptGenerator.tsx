...
import { generateJsonPrompt } from '@/ai/flows/json-prompt-generator';
import { GeneratePromptInput } from '@/ai/flows/json-prompt-generator.types';
...
export function JsonPromptGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateJsonPrompt(input);
      setGeneratedJson(result.prompt);
    } catch (error: any) {
      toast({
        title: 'Generation Failed',
        description: error.message || 'Could not generate a JSON prompt.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  const handleCopy = () => {
    ...
    toast({ title: 'JSON prompt copied to clipboard!'});
  };
    ...
    <div className="space-y-6">
      ...
      {(generatedJson || isLoading) && (
        <div className="pt-4 border-t">
          <Label>Generated JSON Prompt</Label>
          <Card className="mt-2 relative">
            <Button variant="ghost" size="icon" className="absolute top-2 right-2" onClick={handleCopy} disabled={!generatedJson}>
            ...
            <CardContent className="p-0">
                {isLoading ? (
                    ...
                    <SyntaxHighlighter language="json" style={vscDarkPlus} customStyle={{ margin: 0, padding: '1rem', background: 'hsl(var(--muted))' }} wrapLongLines={true}>
