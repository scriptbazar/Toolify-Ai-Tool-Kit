...
import { UploadCloud, Download, ImageIcon, PenLine, Loader2 } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import Image from 'next/image';

interface ImageResizerBoxProps {
    title: string;
...
export function ImageResizerBox({
    title,
    icon: Icon,
    targetWidth,
    targetHeight,
    maxSizeKb,
    dpi,
    downloadPrefix = 'resized'
}: ImageResizerBoxProps) {
  ...
  const handleResize = () => {
    ...
    img.onload = () => {
      ...
      if (dataUrl.length > maxSizeKb * 1024 * 4/3) {
          toast({ title: "Warning", description: `Result may be larger than ${maxSizeKb}KB.`, variant: "default" });
      }
  ...
  const handleDownload = () => {
      ...
      link.download = `${downloadPrefix}-${title.toLowerCase()}-${Date.now()}.jpeg`;
      link.click();
  }
